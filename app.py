...
if selection == "Estadísticas Generales":
    ...
    # Selector de categoría en el contenido principal
    all_categories = pd.read_sql("SELECT DISTINCT category FROM general_statistics", get_keywords_connection())
    selected_category = st.selectbox("Filtrar por Categoría", ['Todas'] + sorted(all_categories['category'].dropna().tolist()))

    # Cargar estadísticas filtradas
    df_stats = load_statistics(category=selected_category)
